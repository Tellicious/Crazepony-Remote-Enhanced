...
uint8_t NRF24_rx_0_address[5]; //RX address
...
uint8_t NRF24_RXMatch(uint64_t* address, uint8_t working_byte, uint32_t delay, uint32_t timeout){
...
while (1) {
  ...
  if (NRF24_available()) {
    ...
    if (millis() > end) {
      ...
      // Check address
      ((*address & add_comp1) == add_comp1) ? (*address &= add_comp2) : (*address += (uint64_t) 1 << shift);
    }
  }
}
